    ...
    if avito_record and domclick_record:
        ...
        for dc_type_name, dc_type_data in domclick_apt_types.items():
            ...
            for idx, dc_apt in enumerate(dc_apartments):
                ...
                has_photos = len(dc_apt.get('photos', [])) > 0
                print(f"      [{idx}] '{dc_title[:60] if dc_title else 'нет title'}...'")
                print(f"          Площадь: {dc_area if dc_area else 'не распознана'}, Этаж: {dc_floor if dc_floor else 'не распознан'}, Фото: {'есть' if has_photos else 'нет'}")
            ...
            for i, dc_apt in enumerate(dc_apartments):
                ...
                print(f"         📐 Парсинг: Площадь={dc_area if dc_area else 'не распознана'}, Этаж={dc_floor if dc_floor else 'не распознан'}")
