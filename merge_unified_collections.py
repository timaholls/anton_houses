#!/usr/bin/env python3
"""
Создает коллекцию unified_houses, объединяя квартиры из unified_houses и unified_houses_2.
Основная логика:
...
TARGET_COLLECTION = "unified_houses"
...
def ensure_completion_date(apartment: Dict[str, Any], fallback: Optional[str]) -> None:
    """Гарантирует наличие completionDate у квартиры, используя fallback при необходимости."""
    if apartment.get("completionDate"):
        return
    if fallback:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Объединяет unified_houses и unified_houses_2 в unified_houses"
    )
    parser.add_argument(
        "--building",
